...
# NOTE this version has been adapted to processing SiMSen-Seq libraries 
...
display_help() {
    ...
    echo "   -c  --trim_demultiplex    Should demultiplexing artefacts be trimmed from fastq name, default is FALSE"
    echo
    exit 1
}
...
fastqc=false
multiqc=false
trim_demultiplex=false

touch log.txt

while getopts ':hfi:b:r:u:s:t:q:p:c' option; do
  case "$option" in
    h | --help)
        display_help
        exit 0
...
do
  ...
  # Check if demultipelxing artefacts should be trimmed from fastq name
  if $trim_demultiplex
    then
      # TODO This removes demultiplexing info from file names.
      # Demultiplexed fastq will contain additional info in the format:
      # <name_on_sample_sheet>_Sx_L00x_Rx_001.fastq.gz
      # this command removed all after '_S'
      sample_name=`echo "$fastq" | sed 's/_S.*.fastq.gz//g'`  
    else
      # If you only want to remove ".fastq.gz", use the following:
      find=".fastq.gz"
      replace=""
